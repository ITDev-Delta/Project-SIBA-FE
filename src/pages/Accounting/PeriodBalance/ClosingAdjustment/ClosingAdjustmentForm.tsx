...
const ClosingAdjustmentJournalForm = () => {
  ...
  const onAddData: SubmitHandler<FieldValues> = async (data) => {
    ...
    await addClosingAdjustJournal(payload)
      .then((res) => {
        setIsLoading(false);
    ...
    await bulkTransaction(payload)
      .then((res) => {
        ...
        if (res.status === 200 || res.status === 201) {
          setIsLoading(false);
          toast.success(res.data.message);
          navigate("/accounting/period-balance/closing-adjustment");
        } else {
          setIsLoading(false);
          toast.error("Failed to add data");
        }
  ...
  return (
    <>
      <div className="sticky top-16 left-0 right-0 flex flex-row justify-between items-center mb-5 bg-white z-40 py-4 ">
        <MyBreadcrumb
          pageName="Closing Adjustment Journal"
          link="/accounting/period-balance/closing-adjustment"
          session="Tambah Data"
        />
        <div
          className={`flex ${journalPost ? "justify-between" : "justify-end"} `}
        >
          {journalPost && (
            <PrimaryButton
              onClick={() => {
...
export default ClosingAdjustmentJournalForm;
