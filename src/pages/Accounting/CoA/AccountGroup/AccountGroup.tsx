...
const AccountGroup: React.FC = () => {
  ...
  const canCreate = hasPermission("account-group-create");
  const canUpdate = hasPermission("account-group-update");
  ...
  const getCoa = (page: number, pageSize: number): void => {
    ...
    // get by level : 1
    const promise = getAccountCoa(3, params);
  ...
  const handleNavigate = () => {
    navigate("/accounting/coa/account-group/add");
  };
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-8">
        <MyBreadcrumb pageName="Account Group" />
        {canCreate && (
      ...
      {isModalOpen && selectedRecord && (
        <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
          <Modal
            ...
            title={
              <>
                {canUpdate ? <>Edit Account Group</> : <>View Account Group</>}
                {canUpdate && (
