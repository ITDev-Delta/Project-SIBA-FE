...
const GeneralJournal: React.FC = () => {
  ...
  const canCreate = hasPermission("general-journal-create");
  const canUpdate = hasPermission("general-journal-update");
      ...
      render: (_: any, record: any) => {
        ...
        return (
          <SelectGroupField
            ...
            disabled={
              selectedRecord?.status === EnumJournal.Posted || !canUpdate
            }
            options={coa}
  ...
  const getJournal = (page: number, pageSize: number): void => {
    ...
    // get by level : 1
    const promise = getGeneralJournal(params);
    promise
      .then((res) => {
  ...
  const getJournalById = (id: string): void => {
    setIsLoadingModal(true);
    const promise = getGeneralJournalById(id);
    promise
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          setIsLoadingModal(false);
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-8">
        <MyBreadcrumb pageName="General Journal" />
        {canCreate && (
          <PrimaryButton to={"/accounting/journal-entry/general-journal/add"}>
            Tambah Data
          </PrimaryButton>
        )}
      </div>
