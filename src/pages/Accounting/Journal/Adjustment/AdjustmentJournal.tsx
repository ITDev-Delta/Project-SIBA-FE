...
import {
  bulkTransaction,
  editAdjustmentJournal,
  getAdjustmentJournal,
  getAdjustmentJournalById,
} from "../../../../api/Accounting/services/adjustmentJournalService";
...
const AdjustmentJournal: React.FC = () => {
  ...
  const canCreate = hasPermission("adjustment-journal-create");
  const canUpdate = hasPermission("adjustment-journal-update");
  ...
  const columnsTrans = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        if (selectedRecord?.status === EnumJournal.Posted) {
          return (
            <div className="text-right">{formatCurrency(record.debit)}</div>
          );
        }
        return (
          <InputField
            id={`debit_${record.id}`}
            defaultValue={formatCurrency(record.debit)}
            type="text"
            disabled={!canUpdate}
            onKeyDown={handleKeyDown}
            onChange={(event) => {
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        if (selectedRecord?.status === EnumJournal.Posted) {
          return (
            <div className="text-right">{formatCurrency(record.kredit)}</div>
          );
        }
        return (
          <InputField
            id={`kredit_${record.id}`}
            defaultValue={formatCurrency(record.kredit)}
            type="text"
            onKeyDown={handleKeyDown}
            disabled={!canUpdate}
            onChange={(event) => {
  ...
  const getJournal = (page: number, pageSize: number): void => {
    ...
    // get by level : 1
    const promise = getAdjustmentJournal(params);
  ...
  const getJournalById = (id: string): void => {
    setIsLoadingModal(true);
    console.log("id", id);
    const promise = getAdjustmentJournalById(id);
  ...
  const onSubmit = (data: FieldValues, status: string) => {
    ...
    const payload = {
      ...
      tanggal_journal: dateJournal || selectedRecord.tanggal_journal,
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-8">
        <MyBreadcrumb pageName="Adjustment Journal" />
        {canCreate && (
          <PrimaryButton
            to={"/accounting/journal-entry/adjustment-journal/add"}
          >
            Tambah Data
          </PrimaryButton>
        )}
