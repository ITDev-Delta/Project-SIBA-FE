...
const AdjustmentJournalForm = () => {
  ...
  const onAddData: SubmitHandler<FieldValues> = async (data) => {
    ...
    await addAdjustmentJournal(payload)
      .then((res) => {
        setIsLoading(false);
    ...
    await bulkTransaction(payload)
      .then((res) => {
        ...
        if (res.status === 200 || res.status === 201) {
          setIsLoading(false);
          toast.success(res.data.message);
          navigate("/accounting/journal-entry/adjustment-journal");
        } else {
          setIsLoading(false);
          toast.error("Failed to add data");
        }
  ...
  return (
    <>
      <div className="sticky top-16 left-0 right-0 flex flex-row justify-between items-center mb-5 bg-white z-40 py-4 ">
        <MyBreadcrumb
          pageName="Adjustment Journal"
          link="/accounting/journal-entry/adjustment-journal"
          session="Tambah Data"
        />
        <div
          className={`flex ${
...
export default AdjustmentJournalForm;
