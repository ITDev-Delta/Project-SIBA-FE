...
import {
  getServiceValutaAsingJournal,
  getServiceValutaAsingJournalById,
  postServiceValutaAsingJournal,
} from "../../../api/Accounting/services/valutaAsingService";
import type { ICoa } from "../../../api/Accounting/types/coa.interface";
import type {
  IValutaAsingJournal,
  IValutaAsingJournalDetail,
  IValutaAsingJournalTrans,
...
const ForeignExchangeJournal: React.FC = () => {
  const {
    register,
    reset,
    getValues,
  ...
  const canUpdate = hasPermission("foreign-exchange-journal-update");
  ...
  const [valutaAsingJournal, setValutaAsingJournal] = useState<
    IValutaAsingJournal[]
  >([]);
  ...
  const [selectedRecord, setSelectedRecord] =
    useState<IValutaAsingJournalDetail>();
  const [transaksi, setTransaksi] = useState<IValutaAsingJournalTrans[]>([]);
  const [tableKey, setTableKey] = useState<number>(0);

  const handleView = (record: IValutaAsingJournalDetail) => {
    console.log(record);
  ...
  const getJournal = (page: number, pageSize: number): void => {
    ...
    const params = {
      ...
      tanggal_create: getValues("tanggal_create_search"),
      tanggal_posting: getValues("tanggal_posting_search"),
      nominal: (getValues("nominal_search") ?? "").replace(/,/g, ""),
      status_filter: searchStatus,
    };

    const promise = getServiceValutaAsingJournal(params);
    promise
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          setIsLoadingModal(false);
          setValutaAsingJournal(res.data.data.data);
          setPagination({
            current: page,
            pageSize: pageSize,
  ...
  const getJournalById = (id: string): void => {
    ...
    const promise = getServiceValutaAsingJournalById(id);
  ...
  const onSubmit = () => {
    ...
    const promise = postServiceValutaAsingJournal(
  ...
  return (
    <>
      <div className="mb-8">
        <MyBreadcrumb pageName="Foreign Exchange Journal" />
      </div>
      <TableApp
        dataSource={valutaAsingJournal}
        columns={columns}
      ...
      {isModalOpen && selectedRecord && (
        <Modal
          ...
          title={
            <>
              <>{`Valuta Asing Journal - ${selectedRecord.data.nomor_journal}`}</>
...
export default ForeignExchangeJournal;
